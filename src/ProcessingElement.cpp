...
void ProcessingElement::rxProcess()
{
	...
	if (req_rx.read() == 1 - current_level_rx) {
		...
		//check which kind of Flit it is
		if(flit_tmp.flit_type == FLIT_TYPE_RESERVE)
		{
			//create a new flit and add it to the queue
			Flit flit;
			flit.src_id = flit_tmp.dst_id;
			flit.dst_id = flit_tmp.src_id;
			flit.timestamp = flit_tmp.timestamp;
			flit.sequence_no = sc_time_stamp().to_double() / GlobalParams::clock_period_ps;
			flit.sequence_length = flit_tmp.sequence_length;
			flit.XY_route = flit_tmp.XY_route;
			flit.L = flit_tmp.L;
			flit.failed_setup = false;
			flit.flit_type = FLIT_TYPE_TEARDOWN;
			flit.hop_no = 0;
			teardown_flit_q.push(flit);
		}
		else if(flit_tmp.flit_type == FLIT_TYPE_TEARDOWN)
		{
			if (flit_tmp.failed_setup == true)
			{
				if(packet_queue.front().retry_count>=3)
					packet_queue.pop();
				else
				{
					packet_queue.front().retry_count++;
					packet_queue.front().send_flit = true;
				}
			}
			else
			{
				//we got a teardown message so we can pop-it off
				packet_queue.pop();
			}
		}
	}
...
void ProcessingElement::txProcess()
{
	...
	if (ack_tx.read() == current_level_tx) {
	    if (!packet_queue.empty() && (flitqueueAtPreviousCycle || teardown_flit_q.empty())&& packet_queue.front().send_flit) {
			Flit flit = nextFlit();	// Generate a new flit
			flit_tx->write(flit);	// Send the generated flit
			current_level_tx = 1 - current_level_tx;	// Negate the old value for Alternating Bit Protocol (ABP)
			req_tx.write(current_level_tx);
			flitqueueAtPreviousCycle = false;
	    }
		else if(!teardown_flit_q.empty())
		{
			Flit tmp_flit = teardown_flit_q.front();
			teardown_flit_q.pop();
			flit_tx->write(tmp_flit);	// Send the generated flit
			current_level_tx = 1 - current_level_tx;	// Negate the old value for Alternating Bit Protocol (ABP)
			req_tx.write(current_level_tx);
			flitqueueAtPreviousCycle = true;
		}
...
Flit ProcessingElement::nextFlit()
{
	...
	flit.failed_setup = false;
    flit.hop_no = 0;
	flit.flit_type = FLIT_TYPE_RESERVE;
    //  flit.payload     = DEFAULT_PAYLOAD;
	packet_queue.front().send_flit = false;
	switch(packet_queue.front().retry_count)
	{
		case 0:
			flit.XY_route = X_FIRST;
			flit.L = L1;
			break;
		case 1:
			flit.XY_route = X_FIRST;
