...
void Router::rxProcess()
{
	...
	for (int i = 0; i < DIRECTIONS + 2; i++) {
		...
		//check what kind of flit it is
		  RouteData route_data;
		  route_data.current_id = local_id;
		  route_data.src_id = received_flit.src_id;
		  route_data.dst_id = received_flit.dst_id;
		  route_data.dir_in = i;
		  route_data.XY_route = received_flit.XY_route; //X_first or Y_first for adaptive XY
		  int o = route(route_data);
		if(received_flit.flit_type == FLIT_TYPE_TEARDOWN)
		{
			if(received_flit.failed_setup==false)
				op_router.dereserve(received_flit.dst_id, received_flit.src_id, received_flit.L);
			else
				op_router.dereserve(received_flit.dst_id, received_flit.src_id, received_flit.L);
			//cout<<"Dereserving "<<received_flit.src_id<<" "<<received_flit.dst_id<<" "<<local_id<<endl;
		}
		else if (received_flit.flit_type == FLIT_TYPE_RESERVE)
		{
			//try reserving here. //doesnt work create a failure packet
			int success =   op_router.reserve(received_flit.src_id, received_flit.dst_id, i, o, received_flit.L);
			//cout<<"Success "<<received_flit.src_id<<" "<<received_flit.dst_id<<endl;
			if(!success)
			{
				received_flit.src_id = route_data.dst_id;
				received_flit.dst_id = route_data.src_id;
				received_flit.flit_type = FLIT_TYPE_TEARDOWN;
				received_flit.failed_setup = true;
			}
			//and send it back wards.
		}
	  ...
	  */
	  ...
	  if (!buffer[i].IsEmpty()) 
		...
		//{
		  ...
		  route_data.dir_in = i;
		  route_data.XY_route = flit.XY_route; //X_first or Y_first for adaptive XY
		  int o = route(route_data);
...
vector < int > Router::routingFunction(const RouteData & route_data)
{
    ...
    return routingAlgorithm->route(this, route_data, route_data.XY_route);
}
