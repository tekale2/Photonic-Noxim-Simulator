...
enum ROUTE_TYPE{X_FIRST,Y_FIRST};
enum Lambda {L1,L2};
...
enum FlitType {
    FLIT_TYPE_HEAD, FLIT_TYPE_BODY, FLIT_TYPE_TAIL, FLIT_TYPE_RESERVE, FLIT_TYPE_TEARDOWN
};
...
struct RouteData {
    ...
    ROUTE_TYPE XY_route; //X_first or Y_first for adaptive XY
};
...
struct Flit {
    ...
    int sequence_length;
    ROUTE_TYPE XY_route; //X_first or Y_first for adaptive XY
    Lambda L; //the chosen wavelength
    double timestamp;		// Unix timestamp at packet generation
    int hop_no;			// Current number of hops from source to destination
    bool use_low_voltage_path;
	bool failed_setup; //boolean value to hold if path setup failed
	...
	return (flit.src_id == src_id && flit.dst_id == dst_id
		...
		&& flit.XY_route == XY_route
		&& flit.L == L
		&& flit.failed_setup == failed_setup  
